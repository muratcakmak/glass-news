...
import { generateThumbnail, generateSimpleThumbnailUrl } from '../transformers/thumbnail';

export async function saveArticleToR2(article: NewsArticle, env: Env): Promise<NewsArticle> {
  try {
    // Generate and upload thumbnail (AI-generated if available)
    const thumbnail = await generateThumbnail(article, env);
    let thumbnailUrl = generateSimpleThumbnailUrl(article);

    if (thumbnail) {
      const thumbnailKey = `thumbnails/${article.id}.png`;
      ...
      thumbnailUrl = `/thumbnails/${article.id}.png`;
    }
    ...
    await env.NEWS_BUCKET.put(articleKey, JSON.stringify(articleWithThumbnail, null, 2), {
      httpMetadata: {
...
async function updateArticleIndex(article: NewsArticle, env: Env): Promise<void> {
  try {
    console.log(`Updating index for article: ${article.id}, source: ${article.source}`);
    ...
    const updatedIndex = [article.id, ...existingIndex.filter(id => id !== article.id)];
    ...
    const updatedGlobalIndex = [article.id, ...globalIndex.filter(id => id !== article.id)];
...
export async function getArticleFromR2(articleId: string, source: string, env: Env): Promise<NewsArticle | null> {
  try {
...
export async function listArticles(source: string | null, limit: number, env: Env): Promise<NewsArticle[]> {
  try {
